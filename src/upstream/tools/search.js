...
function configureSearchTools(server, tokenProvider, connectionProvider, userAgentProvider) {
  server.tool(
    ...
    async ({ searchText, project, repository, path, branch, includeFacets, skip, top }) => {
      ...
      const baseUrl = normalizeCollectionUrl(connection.serverUrl);
      const url = `${baseUrl}/_apis/search/codesearchresults?api-version=${apiVersion}`;
      const requestBody = {
        searchText,
        includeFacets,
        $skip: skip,
  ...
  server.tool(
    ...
    async ({ searchText, project, wiki, includeFacets, skip, top }) => {
      ...
      const connection = await connectionProvider();
      const baseUrl = normalizeCollectionUrl(connection.serverUrl);
      const url = `${baseUrl}/_apis/search/wikisearchresults?api-version=${apiVersion}`;
      const requestBody = {
        searchText,
        includeFacets,
        $skip: skip,
  ...
  server.tool(
    ...
    async ({
      ...
      const connection = await connectionProvider();
      const baseUrl = normalizeCollectionUrl(connection.serverUrl);
      const url = `${baseUrl}/_apis/search/workitemsearchresults?api-version=${apiVersion}`;
      const requestBody = {
        searchText,
        includeFacets,
        $skip: skip,
      ...
      if (!response.ok) {
        throw new Error(
          `Azure DevOps Work Item Search API error: ${response.status} ${response.statusText}`,
        );
      }
      const result = await response.text();
      return {
...
function normalizeCollectionUrl(raw) {
  ...
  if (!value) {
    throw new Error('Azure DevOps collection URL is not available from connection settings.');
  }
  return value;
}
async function fetchCombinedResults(topSearchResults, gitApi) {
  ...
  for (const searchResult of topSearchResults) {
    try {
      ...
      const changeId =
        Array.isArray(searchResult.versions) && searchResult.versions.length > 0
          ? searchResult.versions[0].changeId
          : undefined;
